...
export function assertSourceKind<TKind extends ServiceReadySource["kind"]>(
  ...
  if (!spec.source || typeof spec.source !== "object" || !("kind" in spec.source)) {
    throw new S100Error(
      "invalid-layer-spec",
      `Layer '${spec.id}' must define a '${kind}' source.`,
    );
  }
  ...
  if ((spec.source as ServiceReadySource).kind !== kind) {
    throw new S100Error(
      "invalid-layer-spec",
      `Layer '${spec.id}' must use a '${kind}' source for the Three.js adapter.`,
      spec,
    );
  }
}
...
export const loadJsonSource = async (
  ...
  if (!fetchImpl) {
    throw new S100Error(
      "invalid-layer-spec",
      "A fetch implementation is required for REST JSON sources.",
    );
  }
