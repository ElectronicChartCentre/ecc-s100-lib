...
import {
  ...
  installCesiumCameraOrbitHandler,
  installCesiumCameraPanHandler,
  panCesiumCamera,
  screenSpaceMovementPoint,
} from "../camera/cameraControls.js";
import {
  createWmsParameters,
  createWmsUrlTemplate,
  fillWmsTemplate,
  getProjectedWmsAlphaOptions,
  isProjectedWmsTranslucent,
  normalizeProjectedExtent,
  projectedExtentsOverlap,
  projectedWmsImageSize,
  sameCrs,
  stripQuery,
  subtractProjectedExtent,
  type ProjectedWmsAlphaOptions,
} from "../layers/map/projectedWmsHelpers.js";
...
export class CesiumEngineScene implements EngineScene {
  ...
  setCameraControls(config: CameraControlConfig): void {
    ...
    const vesselGizmoPointGuard = (point: { x: number; y: number }) =>
      screenPointHitsVesselGizmo(this.cesium, this.viewer, point);
